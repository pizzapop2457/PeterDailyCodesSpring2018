...
int main()
{
	...
	ALLEGRO_FONT *font = al_load_ttf_font("font.ttf", 72, 0);
	float paddle1_x = 600;
	float paddle1_y = 250;
	float paddle2_x = 80;
	float paddle2_y = 250;
	...
	while (!doexit)
	{
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			...
			if (PixelCollision(ball_x, ball_y, 32, 32, paddle1_x, paddle1_y, 20, 100) == true) {
				//flip direction
				ball_dx = -ball_dx;
				ball_dy = -ball_dy;
				//play sound
			...
			if (PixelCollision(ball_x, ball_y, 32, 32, paddle2_x, paddle2_y, 20, 100) == true) {
				ball_dx = -ball_dx;
				ball_dy = -ball_dy;
			}
	...
	al_draw_text(font, al_map_rgb(0, 0, 0), 200, 200, ALLEGRO_ALIGN_CENTRE, "Player");
	al_flip_display();
