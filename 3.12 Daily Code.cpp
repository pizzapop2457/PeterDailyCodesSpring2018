...
int main()
{
	...
	ALLEGRO_BITMAP *loliPic = al_load_bitmap("loli.jpg");
	element air;
	element water;
	element fire;
	element earth;
	...
	water.initElement(200, 50, 2, waterPic);
	fire.initElement(400, 50, 3, firePic);
	earth.initElement(600, 50, 4, earthPic);
	loli.initElement(800, 50, 4, loliPic);
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			...
			if (earth.clicked(bouncer_x, bouncer_y))
				if (earth.isLifted() == false)
					...
					earth.PutDown();
			if (loli.clicked(bouncer_x, bouncer_y))
				if (loli.isLifted() == false)
					loli.liftUp();
				else
					loli.PutDown();
		}
